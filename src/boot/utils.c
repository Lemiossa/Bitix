...
void
ntos (u32 val, uchar *out, u16 base, bool neg)
{
	char tmp[32];
...
void
putcat (uchar c, uint x, uint y, uchar attr)
{
...
void
copy_bytes (void *dst, void *src, int n)
{
	uchar *d = (uchar *)dst;
	uchar *s = (uchar *)src;
	int i;
...
int
read_inode (u16 idx, bfx_inode_t *out)
{
	u16 block, offset;
	uchar *inode_ptr;
	int i;
...
u16
find_inode_in_dir (u16 dir_idx, uchar *name)
{
	bfx_inode_t dir;
	u16 pos = 0;
...
int
bfx_readfile (uchar *path, u16 seg, u16 off)
{
	u16 idx;
	bfx_inode_t file;
	u16 size;
...
void
putsxy (int x, int y, const uchar *s, uchar attr)
{
	int i = 0;
	while (s[i]) {
