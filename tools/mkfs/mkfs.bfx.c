 ...
 */
static void
rblock (int fd, u16 block, void *buf)
{
 ...
 */
static void
wblock (int fd, u16 block, void *buf)
{
	lseek (fd, (off_t)block * BLOCK, SEEK_SET);
	write (fd, buf, BLOCK);
}
...
/**
 * Get size with blocks
 */
static u16
size_blocks (u16 size_bytes)
{
 ...
 */
static date16_t
encode_date (int year, int month, int day)
{
	year -= 2000;
	if (year < 0)
		year = 0;
 ...
 */
static void
write_inode (int fd, superblock_t *sb, u16 idx, inode_t *in)
{
	char buf[BLOCK];
 ...
 */
static u16
find_inode_by_name (int fd, superblock_t *sb, u16 dir_idx, const char *name)
{
	char buf[BLOCK];
	...
	for (u16 pos = 0; pos < dir->size; pos += sizeof (u16)) {
		...
		if (strncmp (child->name, name, MAX_NAME) == 0)
			return idx;
	}
	return 0;
}
 ...
 */
static int
add_dir_entry (int fd, superblock_t *sb, u16 dir_idx, u16 child_idx)
{
	char buf[BLOCK] = { 0 };
 ...
 */
static u16
create_dir (int fd, superblock_t *sb, const char *name, u8 perms, u16 parent, int *next_inode, u16 *next_data)
{
	inode_t dir = {
		.size = 0,
		.size_blocks = 0,
 ...
 */
static u16
resolve_path (int fd, superblock_t *sb, const char *path, int create, int *next_inode, u16 *next_data, u8 perms)
{
	if (!path || !*path || path[0] != '/')
		return 0;
...
main (int argc, char **argv)
{
	...
	if (total_blocks > 65534) {
		total_blocks = 65535;
		if (ftruncate (imgfd, total_blocks * BLOCK) < 0) {
			perror ("ftruncate");
			close (imgfd);
			exit (1);
		}
		printf ("\033[32mWARNING: Image truncated to 65535 block (%u bytes)\033[0m\n", total_blocks * BLOCK);
	}
	...
	for (int i = 3; i < argc; i++) {
		...
		*colon = 0;
		...
		u16 blocks_needed = size_blocks (fsize);
		...
		for (u16 j = 0; j < blocks_needed; j++) {
			...
			if (j == blocks_needed - 1) {
				to_read = fsize - j * BLOCK;
			}
			ssize_t n = read (sfd, buf, to_read);
			if (n < to_read) {
				memset (buf + n, 0, BLOCK - n);
			}
